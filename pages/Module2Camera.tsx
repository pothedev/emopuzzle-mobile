...
const Module2Camera: React.FC<{navigation:any, route:any}> = ({navigation, route}) => {
  ...
  const [answerState, setAnswerState] = useState<"correct" | "wrong" | "neutral">("correct")
  ...
  useEffect(() => {
    if (!camEnabled) {
      // if cam is disabled close ws connection
    ...
    const ws = new WebSocket('ws://super-server/ws');

    ws.onopen = () => {
      console.log('WebSocket open');
      ws.send(JSON.stringify({
        emotion: tasks[storyIndex].emotion,
        confidence: 0.3
      }));
    };
    ...
    return () => {
      // close connection once emotion is found
  ...
  const sendFrameToServer = async (arrayBuffer: ArrayBuffer) => {
    try {
      if (wsRef.current?.readyState === WebSocket.OPEN) {
        wsRef.current.send(arrayBuffer); // send raw buffer
      }
    } catch (err) {
  ...
  const frameProcessor = useFrameProcessor((frame) => {
    ...
    try {
      ...
      runOnJS(sendFrameToServer)(buffer);
    } catch (err) {
      console.error("Frame conversion failed", err);
      runOnJS(() => { isProcessing.current = false });
    }
  }, []);
  ...
  useEffect(() => {
    const requestCamPermission = async () => {
      ...
      if (!camEnabled) {console.log('nvm'); return}
        ...
        <View style={{flex: 1}}>
          ...
          <View style={[{ width: '100%', flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center' }, answerState==='wrong' && {justifyContent: 'space-between'}]}>
            ...
            <TouchableOpacity
              ...
              onPress={() => {Object.keys(tasks).length === localStoryIndex ? navigation.navigate('WellDone', {moduleIndex: moduleIndex, moduleTitle: moduleTitle}) : setLocalStoryIndex(localStoryIndex+1)}}
            >
